    ...
    commitPriority(0)
{
    ...
    /* Make FUPipelines for each CVA6FU */
    for (unsigned int i = 0; i < numFuncUnits; i++) {
        std::ostringstream fu_name;
        CVA6FU *fu_description = fuDescriptions.funcUnits[i];
...
Execute::updateBranchData(
    ...
    CVA6DynInstPtr inst, const PCStateBase &target,
    BranchData &branch)
{
    if (reason != BranchData::NoBranch) {
        ...
        branch = BranchData(reason, tid,
             ...
             * courtesy and looks better on cva6view */
...
void
Execute::handleMemResponse(CVA6DynInstPtr inst,
    LSQ::LSQRequestPtr response, BranchData &branch, Fault &fault)
{
    ...
    if (inst->translationFault != NoFault) {
        ...
        DPRINTF(CVA6Mem, "Completing fault from DTLB access: %s\n",
            inst->translationFault->name());

        if (inst->staticInst->isPrefetch()) {
            DPRINTF(CVA6Mem, "Not taking fault on prefetch: %s\n",
                inst->translationFault->name());
        ...
        DPRINTF(CVA6Mem, "Memory response inst: %s addr: 0x%x size: %d\n",
            *inst, packet->getAddr(), packet->getSize());

        if (is_load && packet->getSize() > 0) {
            DPRINTF(CVA6Mem, "Memory data[0]: 0x%x\n",
        ...
        if (fault != NoFault) {
            ...
            DPRINTF(CVA6Mem, "Fault in memory completeAcc: %s\n",
                fault->name());
            fault->invoke(thread, inst->staticInst);
        } else {
...
Execute::takeInterrupt(ThreadID thread_id, BranchData &branch)
{
    ...
    if (interrupt != NoFault) {
        ...
        updateBranchData(thread_id, BranchData::Interrupt,
            CVA6DynInst::bubble(), cpu.getContext(thread_id)->pcState(),
            branch);
    }
...
bool
Execute::executeMemRefInst(CVA6DynInstPtr inst, BranchData &branch,
    bool &passed_predicate, Fault &fault)
{
    ...
    if (!lsq.canRequest()) {
        ...
        DPRINTF(CVA6Execute, "Initiating memRef inst: %s\n", *inst);
        ...
        if (init_fault != NoFault) {
            DPRINTF(CVA6Execute, "Fault on memory inst: %s"
             ...
             * predicate */
            if (!context.readMemAccPredicate()) {
                DPRINTF(CVA6Mem, "No memory access for inst: %s\n", *inst);
...
Execute::issue(ThreadID thread_id)
{
    ...
    do {
        CVA6DynInstPtr inst = insts_in->insts[thread.inputIndex];
        ...
        {
            DPRINTF(CVA6Execute, "Discarding inst: %s from suspended"
                " thread\n", *inst);

            issued = true;
            discarded = true;
        } else if (inst->id.streamSeqNum != thread.streamSeqNum) {
            DPRINTF(CVA6Execute, "Discarding inst: %s as its stream"
            ...
            do {
                ...
                DPRINTF(CVA6Execute, "Trying to issue inst: %s to FU: %d\n",
                    *inst, fu_index);
                ...
                if (inst->isNoCostInst()) {
                    ...
                    DPRINTF(CVA6Execute, "Issuing %s to %d\n",
                    ...
                    /* Skip */
                    if (!fu_is_capable) {
                        DPRINTF(CVA6Execute, "Can't issue as FU: %d isn't"
                            " capable\n", fu_index);
                    } else {
                        DPRINTF(CVA6Execute, "Can't issue as FU: %d is"
                            " already busy\n", fu_index);
                    }
                } else if (fu->stalled) {
                    DPRINTF(CVA6Execute, "Can't issue inst: %s into FU: %d,"
                        " it's stalled\n",
                        *inst, fu_index);
                } else if (!fu->canInsert()) {
                    DPRINTF(CVA6Execute, "Can't issue inst: %s to busy FU"
                        " for another: %d cycles\n",
                        *inst, fu->cyclesBeforeInsert());
                } else {
                    CVA6FUTiming *timing = (!inst->isFault() ?
                    ...
                    if (timing && timing->suppress) {
                        DPRINTF(CVA6Execute, "Can't issue inst: %s as extra"
                    ...
                    {
                        DPRINTF(CVA6Execute, "Can't issue inst: %s yet\n",
                            *inst);
                    } else {
                        /* Can insert the instruction into this FU */
                        DPRINTF(CVA6Execute, "Issuing inst: %s"
                            " into FU %d\n", *inst,
                            fu_index);
                        ...
                        if (issued_mem_ref) {
                            ...
                            if (allowEarlyMemIssue) {
                                ...
                                if (lsq.getLastMemBarrier(thread_id) >
                                    inst->instToWaitFor)
                                {
                                    DPRINTF(CVA6Execute, "A barrier will"
                                    ...
                                    DPRINTF(CVA6Execute, "Memory ref inst:"
                            ...
                            DPRINTF(CVA6Execute, "Pushing mem inst: %s\n",
                                *inst);
            ...
            if (!issued)
                DPRINTF(CVA6Execute, "Didn't issue inst: %s\n", *inst);
        }

        if (issued) {
            /* Generate CVA6Trace's CVA6Inst lines.  Do this at commit
             *  to allow better instruction annotation? */
            if (debug::CVA6Trace && !inst->isBubble()) {
                inst->cva6TraceInst(*this);
            }
            ...
            {
                DPRINTF(CVA6Mem, "Issuing memory barrier inst: %s\n", *inst);
            ...
            if (!discarded && !inst->isBubble()) {
                ...
                if (num_insts_issued == issueLimit)
                    DPRINTF(CVA6Execute, "Reached inst issue limit\n");
            }

            thread.inputIndex++;
            DPRINTF(CVA6Execute, "Stepping to next inst inputIndex: %d\n",
                thread.inputIndex);
        }
...
Execute::doInstCommitAccounting(CVA6DynInstPtr inst)
{
    ...
    CVA6Thread *thread = cpu.threads[inst->id.threadId];
...
bool
Execute::commitInst(CVA6DynInstPtr inst, bool early_memory_issue,
    ...
    bool &completed_mem_issue)
{
        ...
        !isInterrupted(thread_id))
    {
        ...
        DPRINTF(CVA6Execute, "Fault inst reached Execute: %s\n",
            inst->fault->name());
        ...
        if (completed_mem_inst && fault != NoFault) {
            if (early_memory_issue) {
                DPRINTF(CVA6Execute, "Fault in early executing inst: %s\n",
                    fault->name());
                ...
                DPRINTF(CVA6Execute, "Fault in execute: %s\n",
                    fault->name());
    ...
    {
        DPRINTF(CVA6Execute, "Can't commit data barrier inst: %s yet as"
        ...
        DPRINTF(CVA6Execute, "Committing inst: %s\n", *inst);
        ...
        if (fault != NoFault) {
            ...
            DPRINTF(CVA6Execute, "Fault in execute of inst: %s fault: %s\n",
                *inst, fault->name());
            fault->invoke(thread, inst->staticInst);
        }
    ...
    if (completed_inst) {
                ...
                interrupts */
        {
            ...
            DPRINTF(CVA6Interrupt, "Suspending thread: %d from Execute"
                " inst: %s\n", thread_id, *inst);
    ...
    BranchData &branch)
{
    ...
    if (only_commit_microops && !ex_info.inFlightInsts->empty()) {
        DPRINTF(CVA6Interrupt, "Only commit microops %s %d\n",
            *(ex_info.inFlightInsts->front().inst),
            ex_info.lastCommitWasEndOfMacroop);
    }

    /* Some more instructions to process */
    while (!ex_info.inFlightInsts->empty() &&
        ...
        )
    {
        if (only_commit_microops) {
            DPRINTF(CVA6Interrupt, "Committing tail of insts before"
                " interrupt: %s\n",
                *(ex_info.inFlightInsts->front().inst));
        }
        ...
        CVA6DynInstPtr inst = head_inflight_inst->inst;
        ...
        DPRINTF(CVA6Execute, "Trying to commit canCommitInsts: %d\n",
            can_commit_insts);
        ...
        if (isInbetweenInsts(thread_id) && tryPCEvents(thread_id)) {
            ...
            updateBranchData(thread_id, BranchData::UnpredictedBranch,
                CVA6DynInst::bubble(), thread->pcState(), branch);
            ...
            num_mem_refs_committed < memoryCommitLimit)
        {
            ...
            DPRINTF(CVA6Execute, "Trying to commit mem response: %s\n",
                *inst);

            /* Complete or discard the response */
            if (discard_inst) {
                DPRINTF(CVA6Execute, "Discarding mem inst: %s as its"
            ...
            if (!ex_info.inFUMemInsts->empty() && lsq.canRequest()) {
                DPRINTF(CVA6Execute, "Trying to commit from mem FUs\n");

                const CVA6DynInstPtr head_mem_ref_inst =
                    ex_info.inFUMemInsts->front().inst;
                FUPipeline *fu = funcUnits[head_mem_ref_inst->fuIndex];
                const CVA6DynInstPtr &fu_inst = fu->front().inst;
                ...
                {
                    DPRINTF(CVA6Execute, "Issuing mem ref early"
            ...
            if (!completed_inst && inst->isNoCostInst()) {
                DPRINTF(CVA6Execute, "Committing no cost inst: %s", *inst);
            ...
            if (!completed_inst && !inst->inLSQ) {
                DPRINTF(CVA6Execute, "Trying to commit from FUs\n");
            ...
            if (try_to_commit) {
                ...
                if (!discard_inst) {
                    ...
                    if (inst->extraCommitDelayExpr) {
                        DPRINTF(CVA6Execute, "Evaluating expression for"
                        ...
                        DPRINTF(CVA6Execute, "Extra commit delay expr"
                            " result: %d\n", extra_delay);

                        if (extra_delay < 128) {
                            inst->extraCommitDelay += Cycles(extra_delay);
                        } else {
                            DPRINTF(CVA6Execute, "Extra commit delay was"
                                " very long: %d\n", extra_delay);
                        }
                        inst->extraCommitDelayExpr = NULL;
                    }
                    ...
                    {
                        DPRINTF(CVA6Execute, "Not committing inst: %s yet"
                ...
                if (completed_inst) {
                    ...
                    if (inst->fuIndex != noCostFUIndex) {
                        DPRINTF(CVA6Execute, "Unstalling %d for inst %s\n",
            ...
            DPRINTF(CVA6Execute, "No instructions to commit\n");
            completed_inst = false;
        }
        ...
        if (completed_inst && !(issued_mem_ref && fault == NoFault)) {
            ...
            DPRINTF(CVA6Execute, "Completed inst: %s\n", *inst);
             ...
             *  value from changing too much on the cva6view display */
            ...
            if (inst->isInst() && inst->staticInst->isFullMemBarrier()) {
                DPRINTF(CVA6Mem, "Completing memory barrier"
                    " inst: %s committed: %d\n", *inst, committed_inst);
        ...
        if (committed_inst) {
             ...
             *  slot */
            if (debug::CVA6Trace && !is_no_cost_inst)
            ...
            if (num_insts_committed == commitLimit)
                DPRINTF(CVA6Execute, "Reached inst commit limit\n");
            ...
            if (num_mem_refs_committed == memoryCommitLimit)
                DPRINTF(CVA6Execute, "Reached mem ref commit limit\n");
        }
    }
}
...
Execute::evaluate()
{
    ...
    if (interrupt_tid != InvalidThreadID) {
        ...
        DPRINTF(CVA6Interrupt, "Execute skipping a cycle to allow old"
            " branch to complete\n");
    } else {
        ...
        if (commit_tid != InvalidThreadID) {
            ...
            DPRINTF(CVA6Execute, "Attempting to commit [tid:%d]\n",
                    commit_tid);
            /* commit can set stalled flags observable
             *  to issue and so *must* be
             *  called first */
            ...
            /* Halt fetch, but don't do it until
             * we have the current instruction in
             *  the bag */
        ...
        if (issue_tid != InvalidThreadID) {
            DPRINTF(CVA6Execute, "Attempting to issue [tid:%d]\n",
                    issue_tid);
     ...
     * clock cycle */
    std::vector<CVA6DynInstPtr> next_issuable_insts;
    ...
    for (ThreadID tid = 0; tid < cpu.numThreads; tid++) {
        ...
        if (getInput(tid)) {
            ...
            CVA6DynInstPtr inst = getInput(tid)->insts[input_index];
            if (inst->isFault()) {
                can_issue_next = true;
...
Execute::checkInterrupts(BranchData& branch, bool& interrupted)
{
    ...
    do {
        ...
        if (FullSystem && cpu.getInterruptController(tid)) {
            ...
            DPRINTF(CVA6Interrupt, "No interrupt controller\n");
        }
        DPRINTF(CVA6Interrupt,
...
Execute::cva6Trace() const
{
    ...
    lsq.cva6Trace();
    inputBuffer[0].cva6Trace();
    scoreboard[0].cva6Trace();
    ...
    cva6::cva6Trace("insts=%s inputIndex=%d streamSeqNum=%d"
    ...
    std::for_each(funcUnits.begin(), funcUnits.end(),
        std::mem_fn(&FUPipeline::cva6Trace));

    executeInfo[0].inFlightInsts->cva6Trace();
    executeInfo[0].inFUMemInsts->cva6Trace();
}
...
Execute::drainResume()
{
    DPRINTF(Drain, "CVA6Execute drainResume\n");
...
Execute::drain()
{
    DPRINTF(Drain, "CVA6Execute drain\n");
...
bool
Execute::instIsRightStream(CVA6DynInstPtr inst)
{
...
bool
Execute::instIsHeadInst(CVA6DynInstPtr inst)
{
    bool ret = false;
