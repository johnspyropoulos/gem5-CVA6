...
Fetch1::handleTLBResponse(FetchRequestPtr response)
{
    ...
    if (response->fault != NoFault) {
        ...
        if (debug::CVA6Trace)
            cva6TraceResponseLine(name(), response);
    } else {
...
/** Print the appropriate CVA6Line line for a fetch response */
void
Fetch1::cva6TraceResponseLine(const std::string &name,
    Fetch1::FetchRequestPtr response) const
{
    const RequestPtr &request = response->request;

    if (response->packet && response->packet->isError()) {
        cva6Line(*this, "id=F;%s vaddr=0x%x fault=\"error packet\"\n",
            response->id, request->getVaddr());
    } else if (response->fault != NoFault) {
        cva6Line(*this, "id=F;%s vaddr=0x%x fault=\"%s\"\n",
            response->id, request->getVaddr(), response->fault->name());
    } else {
        cva6Line(*this, "id=%s size=%d vaddr=0x%x paddr=0x%x\n",
...
Fetch1::recvTimingResp(PacketPtr response)
{
    ...
    if (debug::CVA6Trace)
        cva6TraceResponseLine(name(), fetch_request);
...
Fetch1::evaluate()
{
    ...
    if (execute_branch.threadId != InvalidThreadID &&
        ...
        if (execute_branch.isStreamChange()) {
            ...
            /* Handle branch predictions by
             * changing the instruction source
             * if we're still processing the
             * same stream (as set by streamSeqNum)
...
void
Fetch1::cva6Trace() const
{
    // TODO: Un-bork cva6Trace for THREADS
    // bork bork bork
    ...
    cva6::cva6Trace("state=%s icacheState=%s in_tlb_mem=%s/%s"
