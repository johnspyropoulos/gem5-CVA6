...
{
    CVA6DynInstPtr inst = branch.inst;
...
Fetch2::evaluate()
{
    ...
    if (tid != InvalidThreadID) {
            ...
            prediction.isBubble() /* No predicted branch */)
        {
             ...
             *  issues */
            bool set_pc = fetch_info.lastStreamSeqNum
                != line_in->id.streamSeqNum;
            ...
            if (discard_line) {
                ...
                /* Pack a fault as a CVA6DynInst with ->fault set */

                /* Make a new instruction and pick up the line, stream,
                 *  prediction, thread ids from the incoming line */
                dyn_inst = new CVA6DynInst(nullStaticInstPtr, line_in->id);
                ...
                if (decoder->instReady()) {
                    ...
                    /* Make a new instruction and pick up the line, stream,
                     *  prediction, thread ids from the incoming line */
                    dyn_inst = new CVA6DynInst(decoded_inst, line_in->id);
                     ...
                     * as CVA6 has micro-op decomposition in a separate
            ...
            if (dyn_inst) {
                ...
                /* Output CVA6Trace instruction info for
                 *  pre-microop decomposition macroops */
                if (debug::CVA6Trace && !dyn_inst->isFault() &&
                    dyn_inst->staticInst->isMacroop()) {
                    dyn_inst->cva6TraceInst(*this);
                }
            }
...
Fetch2::Fetch2Stats::Fetch2Stats(CVA6CPU *cpu)
...
Fetch2::cva6Trace() const
{
    ...
    cva6::cva6Trace("inputIndex=%d havePC=%d predictionSeqNum=%d insts=%s\n",
